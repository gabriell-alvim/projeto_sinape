...
4. Identificar especificamente os itens de licitação que ENTRARAM na subpasta "05 - Finalizado" nesta execução (presentes agora na lista de "05 - Finalizado" e ausentes dela no snapshot anterior — não importa em qual outra subpasta de status estavam antes, ou se são totalmente novos). Esses são candidatos a "dossiê pronto para montar": a documentação de habilitação normalmente já está completa quando um processo chega nesse status.
...
Para cada item identificado no item 4 de "O QUE FAZER EM CADA EXECUÇÃO" acima (entrou em "05 - Finalizado"), tente achar o processo correspondente no Painel:
...
Isso continua sendo modo SUGESTÃO: mesmo encontrando o id com confiança, não chame o Montador de Dossiê nem qualquer endpoint que baixe ou monte documentos — só resolva a identificação para citar no relatório.
